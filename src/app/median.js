...
function MedianFilter() {
	...
	this.convertImage = function (imageData, w,h) {
		...
		var hs = this.calculateHistograms(data), bins=this.NUMCLUSTERS;
		...
		/* element: filter mask consisting of 0 and 1:
		 001100
		 011110
		 111111
		 111111
		...
		*/
		...
		var pxl = evalPixel(255, 255, 255) + 1,
			hst = (this.mode == this.FAST) ? new MedianHistogramFast(pxl) : new MedianHistogram(pxl);
		...
		for (var y = 0; y < h; y++) {
			...
			var removes = this.get2D(0, 2 * padding + 1, Array), i, j, k, r, g, b, x;
	...
	// e.g. pixel[-1] = pixel[0], pixel[-2] = pixel[1]
...
function MedianHistogram(size) {
	...
	this.addItem = function(index, r, g, b) {
		// if added value is below the median, decrease median's position
	...
	this.removeItem = function(index) {
		// if removed value is below the median, raise median's position
		if (index < this.medianbin) {
			this.medianindex++;
		}
