	...
	for (var i=0, ii=layers.length; i < ii; ++i) {
		...
		if (title) {
			...
			if (this.groups[group] && this.groups[group].exclusive === true) {
				$('<input />', {type: 'radio', name: group, value: title, checked:layer.getVisible()}).change([map, layer, group], function(evt) {
					var map = evt.data[0];
					var layers = map.getLayers().getArray();
					for (var i = 0, ii = layers.length; i < ii; ++i) {
				...
				$('<input />', {type: 'checkbox', checked: layer.getVisible()}).change(layer, function(evt) {
					evt.data.setVisible($(this).is(':checked'));
				}).appendTo(item);
				// set title
				$('<span />').html(title).appendTo(item);
				// delete layer button
				var deletable = layer.get("deletable");
				if ((deletable != undefined) && (deletable === true)) { // if can be deleted from layers list
					$('<input />', {type: 'button', 'class': "delete", value: '×'}).click([map, layer], function(evt) {
						var map = evt.data[0];
						var layer = evt.data[1];
...
app.LayersControl.prototype.clear = function() {
	var element = document.getElementsByClassName('layers-control ol-unselectable')[0];
	element.innerHTML = "";
	this.containers = {};
	for (var group in this.groups) {
		this.containers[group] = document.createElement('ul');
